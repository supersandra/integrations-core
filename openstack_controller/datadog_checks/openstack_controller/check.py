...
from datadog_checks.openstack_controller.http_error import ComponentNotFoundError
from datadog_checks.openstack_controller.metrics import (
    HYPERVISOR_SERVICE_CHECK,
    KEYSTONE_DOMAINS_COUNT,
    KEYSTONE_GROUPS_COUNT,
    KEYSTONE_LIMITS_COUNT,
...
class OpenStackControllerCheck(AgentCheck):
    def __init__(self, name, init_config, instances):
        ...
        self.reported_identity_metrics = False
        self.reported_domain_metrics = False
    ...
    def check(self, _instance):
        ...
        if self._authorize(api, tags):
            self.log.info("User successfully authorized")
            self._report_metrics(api, tags)

    def _authorize(self, api, tags):
        try:
            api.authorize()
        except Exception as e:
            self.log.error("Exception while authorizing user: %s", e)
            return False
        else:
            return True
    ...
    def _report_metrics(self, api, tags):
        ...
        self.log.trace("auth_projects: %s", auth_projects)
        for project in auth_projects:
            project_tags = tags + [f"project_id:{project['id']}", f"project_name:{project['name']}"]
            api.set_current_project(project['id'])
            api.authorize()
            self._report_identity_metrics(api, tags)
            self._report_compute_metrics(api, project['id'], tags, project_tags)
            self._report_network_metrics(api, project['id'], tags, project_tags)

    def _report_identity_metrics(self, api, tags):
        if api.component_in_catalog(ComponentType.IDENTITY):
            self.log.debug("`%s` component found in catalog", ComponentType.IDENTITY.value)
            ...
            self.log.warning("`%s` component not found in catalog", ComponentType.IDENTITY.value)

    def _report_compute_metrics(self, api, project_id, tags, project_tags):
        if api.component_in_catalog(ComponentType.COMPUTE):
            self.log.debug("`%s` component found in catalog", ComponentType.COMPUTE.value)
            ...
            self._report_compute_quota_sets(api, project_id, project_tags)
            self._report_compute_servers(api, project_id, project_tags)
        else:
            self.log.warning("`%s` component not found in catalog", ComponentType.COMPUTE.value)

    def _report_network_metrics(self, api, project_id, tags, project_tags):
        if api.component_in_catalog(ComponentType.NETWORK):
            self.log.debug("`%s` component found in catalog", ComponentType.NETWORK.value)
            self._report_network_response_time(api, tags)
        else:
            self.log.warning("`%s` component not found in catalog", ComponentType.NETWORK.value)

    def _report_identity_response_time(self, api, tags):
        self.log.debug("getting `%s` response time", ComponentType.IDENTITY.value)
        self._report_response_time(api, ComponentType.IDENTITY, KEYSTONE_RESPONSE_TIME, KEYSTONE_SERVICE_CHECK, tags)

    def _report_compute_response_time(self, api, tags):
        self.log.debug("getting `%s` response time", ComponentType.COMPUTE.value)
        self._report_response_time(api, ComponentType.COMPUTE, NOVA_RESPONSE_TIME, NOVA_SERVICE_CHECK, tags)

    def _report_network_response_time(self, api, tags):
        self.log.debug("getting `%s` response time", ComponentType.NETWORK.value)
        self._report_response_time(api, ComponentType.NETWORK, NEUTRON_RESPONSE_TIME, NEUTRON_SERVICE_CHECK, tags)

    def _report_response_time(self, api, component_type, metric, service_check, tags):
        try:
            response_time = api.get_response_time(component_type)
            self.log.trace("`%s` response time: %s", component_type.value, response_time)
            if response_time is not None:
                self.gauge(metric, response_time, tags=tags)
                self.service_check(service_check, AgentCheck.OK, tags=tags)
        except Exception as e:
            self.log.error("Exception while getting response time: %s", e)
    ...
    def _report_identity_domains(self, api, tags):
        try:
            ...
            if identity_domains is not None:
                ...
                for domain_id, domain_data in identity_domains.items():
                    ...
                    for metric, value in domain_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + domain_tags)
        except Exception as e:
            self.log.error("Exception while getting identity domains: %s", e)
    ...
    def _report_identity_projects(self, api, tags):
        try:
            ...
            if identity_projects is not None:
                self.gauge(KEYSTONE_PROJECTS_COUNT, len(identity_projects), tags=tags)
                for project_id, project_data in identity_projects.items():
                    project_tags = [
                        'domain_id:{}'.format(project_data['domain_id']),
                        'project_id:{}'.format(project_id),
                        'project_name:{}'.format(project_data['name']),
                    ] + project_data['tags']
                    for metric, value in project_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + project_tags)
        except Exception as e:
            self.log.error("Exception while getting identity projects: %s", e)
    ...
    def _report_identity_users(self, api, tags):
        try:
            ...
            if identity_users is not None:
                self.gauge(KEYSTONE_USERS_COUNT, len(identity_users), tags=tags)
                for user_id, user_data in identity_users.items():
                    user_tags = [
                        'domain_id:{}'.format(user_data['domain_id']),
                        'user_id:{}'.format(user_id),
                        'user_name:{}'.format(user_data['name']),
                    ]
                    for metric, value in user_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + user_tags)
        except Exception as e:
            self.log.error("Exception while getting identity users: %s", e)
    ...
    def _report_identity_groups(self, api, tags):
        try:
            ...
            if identity_groups is not None:
                ...
                for group_id, group_data in identity_groups.items():
                    group_tags = [
                        'domain_id:{}'.format(group_data['domain_id']),
                        'group_id:{}'.format(group_id),
                        'group_name:{}'.format(group_data['name']),
                    ]
                    try:
                        identity_group_users = api.get_identity_group_users(group_id)
                        self.log.debug("identity_group_users: %s", identity_group_users)
                        self.gauge('openstack.keystone.groups.users', len(identity_group_users), tags + group_tags)
                    except Exception as e:
                        self.log.error("Exception while getting identity group `%s`users: %s", group_id, e)
        except Exception as e:
            self.log.error("Exception while getting identity groups: %s", e)
    ...
    def _report_identity_services(self, api, tags):
        try:
            ...
            if identity_services is not None:
                self.gauge(KEYSTONE_SERVICES_COUNT, len(identity_services), tags=tags)
                for service_id, service_data in identity_services.items():
                    service_tags = [
                        'service_id:{}'.format(service_id),
                        'service_name:{}'.format(service_data['name']),
                        'service_type:{}'.format(service_data['type']),
                    ]
                    for metric, value in service_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + service_tags)
        except Exception as e:
            self.log.error("Exception while getting identity services: %s", e)
    ...
    def _report_identity_limits(self, api, tags):
        try:
            ...
            if identity_limits is not None:
                self.gauge(KEYSTONE_LIMITS_COUNT, len(identity_limits), tags=tags)
                for limit_id, limit_data in identity_limits.items():
                    domain_id = limit_data.get('domain_id')
                    project_id = limit_data.get('project_id')
                    region_id = limit_data.get('project_id')
                    limit_tags = [
                        'limit_id:{}'.format(limit_id),
                        'resource_name:{}'.format(limit_data['resource_name']),
                        'service_id:{}'.format(limit_data['service_id']),
                        'region_id:{}'.format(region_id) if region_id else None,
                        'domain_id:{}'.format(domain_id) if domain_id else None,
                        'project_id:{}'.format(project_id) if project_id else None,
                    ]
                    for metric, value in limit_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + limit_tags)
        except Exception as e:
            self.log.error("Exception while getting identity limits: %s", e)
    ...
    def _report_compute_limits(self, api, tags):
        try:
            ...
            self.log.error("Exception while getting compute limits: %s", e)
    ...
    def _report_compute_services(self, api, tags):
        try:
            ...
            if compute_services is not None:
                for service_id, service_data in compute_services.items():
                    service_tags = [
                        'service_id:{}'.format(service_id),
                        'service_name:{}'.format(service_data['name']),
                        'service_host:{}'.format(service_data['host']),
                        'service_status:{}'.format(service_data['status']),
                        'service_state:{}'.format(service_data['state']),
                        'availability_zone:{}'.format(service_data['zone']),
                    ]
                    for metric, value in service_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + service_tags)
        except Exception as e:
            self.log.error("Exception while getting compute services: %s", e)
        ...
        try:
            ...
            if compute_flavors is not None:
                for flavor_id, flavor_data in compute_flavors.items():
                    flavor_tags = [
                        'flavor_id:{}'.format(flavor_id),
                        'flavor_name:{}'.format(flavor_data['name']),
                    ]
                    for metric, value in flavor_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + flavor_tags)
        except Exception as e:
            self.log.error("Exception while getting compute flavors: %s", e)
    ...
    def _report_compute_hypervisors(self, api, tags):
        try:
            ...
            if compute_hypervisors is not None:
                for hypervisor_id, hypervisor_data in compute_hypervisors.items():
                    hypervisor_tags = _create_hypervisor_metric_tags(
                        hypervisor_id, hypervisor_data, compute_os_aggregates
                    )
                    for metric, value in hypervisor_data['metrics'].items():
                        self.gauge(metric, value, tags=tags + hypervisor_tags)
                    self.gauge(
                        NOVA_HYPERVISOR_SERVICE_CHECK,
                        1 if hypervisor_data.get('state') == 'up' else 0,
                        tags=tags + hypervisor_tags,
                    )
        except Exception as e:
            self.log.error("Exception while getting compute hypervisors: %s", e)

    def _report_compute_quota_sets(self, api, project_id, project_tags):
        try:
            compute_quota_sets = api.get_compute_quota_sets(project_id)
            self.log.debug("compute_quota_sets: %s", compute_quota_sets)
            if compute_quota_sets is not None:
                for quota_set_id, quota_set_data in compute_quota_sets.items():
                    quota_set_tags = ['quota_id:{}'.format(quota_set_id)]
                    for metric, value in quota_set_data['metrics'].items():
                        self.gauge(metric, value, tags=project_tags + quota_set_tags)
        except Exception as e:
            self.log.error("Exception while getting compute quota sets: %s", e)
    ...
    def _report_compute_servers(self, api, project_id, project_tags):
        try:
            ...
            if compute_servers is not None:
                self.gauge('openstack.nova.server.count', len(compute_servers), tags=project_tags)
                    ...
                    if server_data["status"] == "active" or server_data["status"] == "error":
                        self.gauge(
                            ...
                            + _create_nova_server_tags(
                                ...
                                server_data.get("hypervisor_hostname"),
                                server_data.get("instance_hostname"),
                                server_data.get("flavor_name"),
                            ),
                        )
        except Exception as e:
            self.log.error("Exception while getting compute servers: %s", e)
    ...
    #     if not self.reported_domain_metrics:
    ...
    #         self.reported_domain_metrics = True
    #
    ...
    #
    # def _report_block_storage_response_time(self, api, tags):
    #     response_time = api.get_block_storage_response_time()
    #     self.log.debug("block storage response time: %s", response_time)
    #     if response_time is not None:
    #         self.gauge('openstack.cinder.response_time', response_time, tags=tags)
    #         self.service_check('openstack.cinder.api.up', AgentCheck.OK, tags=tags)
    #     else:
    #         self.service_check('openstack.cinder.api.up', AgentCheck.UNKNOWN, tags=tags)
    #
