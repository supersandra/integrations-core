...
class ReproduceError(object):
    ...
    def test_query(self, job_name):
        queries_for_dbname = {
            "postgres": ["SELECT pg_sleep(3);", "select * from pg_stat_activity;", "select * from pg_stat_statements;"],
            "dbmorders": ["SELECT pg_sleep(3);", "select * from pg_stat_activity;", "select * from pg_stat_statements;"],
        }
        random_query = random.choice(queries_for_dbname.get("postgres"))
        ...
        if job_name == "job-three":
            ...
            random_query = random.choice(queries_for_dbname.get(dbname))
            with self.db_pool.get_connection(dbname, 60000) as conn:
    ...
    def generate_managed_identity_token(self, client_id: str, identity_scope: str = None):
    ...
    def _new_connection(self, dbname, min_pool_size=1, max_pool_size=None):
        args = {"autocommit": True, "cursor_factory": psycopg.ClientCursor}
        password = self.generate_managed_identity_token(client_id="c8d2cd7d-3e43-46cb-9bd9-050706399f52", identity_scope=DEFAULT_PERMISSION_SCOPE)
        print(password)
        conn_args = {
            'host': "dbm-postgres13-flex-server.postgres.database.azure.com",
            'user': "dbm-datadog-test-identity",
            'password': password,
            'dbname': dbname,
        }
